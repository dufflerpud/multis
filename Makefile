...
again:
		$(MAKE) spotless CC=$(CC) CROSS_COMPILER=$(CROSS_COMPILER) INSTALL_DIR=$(INSTALL_DIR)
		$(MAKE) all CC=$(CC) CROSS_COMPILER=$(CROSS_COMPILER) INSTALL_DIR=$(INSTALL_DIR)
...
ifeq ($(F77BIN),)
    ...
    f77s:
			...
			$(MAKE) $(foreach F,$(BASIC_PROGS),$(F77S_DIR)/$(F).cluster) CC=$(CC) CROSS_COMPILER=$(CROSS_COMPILER) INSTALL_DIR=$(INSTALL_DIR)
endif
...
ifeq ($(F2CBIN),)
    ...
    f2cs:
			@echo "Attempting to build $@:"
			$(MAKE) $(foreach F,$(BASIC_PROGS),$(F2CS_DIR)/$(F).cluster) CC=$(CC) CROSS_COMPILER=$(CROSS_COMPILER) INSTALL_DIR=$(INSTALL_DIR)

    f2ccs:
			date
			@echo "Attempting to build $@:"
			$(MAKE) $(foreach F,$(BASIC_PROGS),$(F2CCS_DIR)/$(F).cluster) CC=$(CC) CROSS_COMPILER=$(CROSS_COMPILER) INSTALL_DIR=$(INSTALL_DIR)
endif

dec10:
			@echo "Attempting to build $@:"
			$(MAKE) $(foreach F,$(BASIC_PROGS),$(DEC10_DIR)/$(F).cluster) CC=$(CC) CROSS_COMPILER=$(CROSS_COMPILER) INSTALL_DIR=$(INSTALL_DIR)

fixed_fs:
			@echo "Attempting to build $@:"
			$(MAKE) $(foreach F,$(BASIC_PROGS),$(FIXED_FS_DIR)/$(F).f) CC=$(CC) CROSS_COMPILER=$(CROSS_COMPILER) INSTALL_DIR=$(INSTALL_DIR)
...
export:			clean
			$(MAKE) f2ccs CC=$(CC) CROSS_COMPILER=$(CROSS_COMPILER) INSTALL_DIR=$(INSTALL_DIR)
...
install:
			$(MAKE) install_one CC=$(CC) CROSS_COMPILER=$(CROSS_COMPILER) INSTALL_DIR=$(INSTALL_DIR)
ifneq (,$(shell command -v x86_64-w64-mingw32-gcc))
			$(MAKE) install_one CC=x86_64-w64-mingw32-gcc CROSS_COMPILER=x86_64-w64-mingw32-gcc INSTALL_DIR=$(USRLOCAL)/wccmultis
endif
...
ifeq ($(ROOT_PRIVS),)
			sudo $(MAKE) $@ CC=$(CC) CROSS_COMPILER=$(CROSS_COMPILER) INSTALL_DIR=$(INSTALL_DIR)
else
			$(RM) -rf $(INSTALL_DIR)
...
%:
		...
		@$(MAKE) ORIGINAL_TARGET=$@ std_$@ CC=$(CC) CROSS_COMPILER=$(CROSS_COMPILER) INSTALL_DIR=$(INSTALL_DIR)
