/************************************************************************
 *indx#	hipak.c - C version of original Macro-10 assembler support for multi-user games
 *@HDR@	$Id$
 *@HDR@
 ...
 ************************************************************************
 *doc#	hipak.c - C version of original Macro-10 assembler support for multi-user games
 ************************************************************************/
#undef	DEBUG_WINDOWS
#undef	DEBUG_HISEG
#undef	DEBUG_DAEMON
...
static int webserver_portnum = -1;
...
static char *webserver_file = NULL;
static int webserver_file_length = 0;
 ...
 ************************************************************************/
	...
	if( daemon_port )
	    fprintf(debug_file,"Daemon");
	else if( remote_host_name )
	    if( webserver_portnum >= 0 )
		fprintf(debug_file,"%s:%s Web %d",
		    remote_host_name,remote_port,webserver_portnum);
	    else
		fprintf(debug_file,"%s:%s",remote_host_name,remote_port);
	else if( webserver_portnum >= 0 )
	    fprintf(debug_file,"Web %d",webserver_portnum);
	else
	    fprintf(debug_file,"Standalone");
 ...
 ************************************************************************/
    ...
    *privd = logical( getuid() == PRIVILEGED_UID );
...
typedef enum { DT_curses, DT_array, DT_table } display_type;
struct display_struct
    {
    char		d_nrows, d_ncols;		/* Screen dimensions */
    char		d_row, d_col;			/* Current position */
    char		d_fg, d_bg;			/* Current colors */
    display_type	d_display_type;			/* What display type */
 ...
 ************************************************************************/
	...
	case DT_table:
	    ...
	    integer fortran_row, fortran_col;
	    integer space = ' ';	/* With default color scheme */
	    for( fortran_row=curd->d_nrows; fortran_row>0; fortran_row-- )
		for( fortran_col=curd->d_ncols; fortran_col>0; fortran_col-- )
		    {
 ...
 ************************************************************************/
    ...
    switch( curd->d_display_type )
        {
	case DT_curses:		wclear( curd->d_win );		break;
	default:		fwerase( curd );		break;
	}
    }
 ...
 ************************************************************************/
    ...
    switch( curd->d_display_type )
        {
	case DT_curses:		wmove(curd->d_win,row,col);	break;
	default:						break;
	}
    }
 ...
 ************************************************************************/
void init_colors( struct display_struct *curd )
    {
    int fg, bg;
    int max_pair = 0;
    if( curd->d_display_type == DT_curses ) start_color();
    for( fg=NCOLORS; --fg>=0; )
	for( bg=0; bg<NCOLORS; bg++ )
	    {
	    single_to_pair_map[fg][bg] = max_pair++;
	    if( curd->d_display_type == DT_curses )
		{
	        if( max_pair <= COLOR_PAIRS )
		    init_pair( single_to_pair_map[fg][bg], fg, bg );
		else
...
/************************************************************************
 *	Code needs to turn a display number into a pointer to the	*
 #	structure with information about it.  If the structure hasn't	*
 *	hasn't been properly initialized, set it up.			*
 *									*
 *	screen_num>0		display_type=DT_array			*
 *	screen_num==0							*
 *	    webserver_chan==0	display_type=DT_curses			*
 *	    webserver_chan>0	display_type=DT_array (for webserver)	*
 ************************************************************************/
    {
	...
	if( screen_num == 0 )
	    ...
	    if( webserver_chan )
		{
		curd->d_display_type = DT_array;
		curd->d_data =
		    (chtype*)malloc(sizeof(chtype)*curd->d_nrows*curd->d_ncols);
		}
	    else
		{
		curd->d_display_type = DT_curses;
		curd->d_win = initscr();
		}
	...
	else if( curd->d_table )
	    curd->d_display_type = DT_table;
	else
	    {
	    curd->d_display_type = DT_array;
	    curd->d_data =
	...
	if( curd->d_display_type != DT_table )
	    fwerase( curd );
	fwmove( curd, 0, 0 );
	}
    return curd;
 ...
 ************************************************************************/
	...
	displays[*winnum].d_table = *address;
    else
 ...
 ************************************************************************/
    ...
    switch( curd->d_display_type )
        {
	case DT_curses:		wrefresh( curd->d_win );	break;
	default:						break;
	}
    }
 ...
 ************************************************************************/
    ...
    switch( curd->d_display_type )
        {
	case DT_curses:		redrawwin( curd->d_win );	break;
	default:						break;
	}
    }

/************************************************************************/
void fwaddpch( struct display_struct *curd, chtype cell )
/************************************************************************/
    {
 ...
 ************************************************************************/
    ...
    if( ! in_graphics_mode )
	{
	(void)get_display(default_display);
	in_graphics_mode = 1;
	}
    }
...
extern int chrat( struct display_struct *curd, int row, int col );
/************************************************************************/
void update_( integer *winnum )
/************************************************************************
 * Copy virtual screen *winnum to actual screen 0.			*
 * Used in: greebl risk							*
 ************************************************************************/
    {
    struct display_struct *todp = get_display(0);
    if( *winnum > 0 )
	{
...
/************************************************************************/
int chrat( struct display_struct *curd, int row, int col )
/************************************************************************
 *	Return the character currently display at row, col.		*
 ************************************************************************/
    {
    integer res;
	...
	case DT_curses:
	    ...
	    res = mvwinch( curd->d_win, row, col );
	    wmove( curd->d_win, oldrow, oldcol );
	    }
	    break;
	case DT_array:
	    {
	    res = curd->d_data[ curd->d_ncols*row + col ];
	    }
	    break;
	case DT_table:
	    {
	    ...
	    integer line = __LINE__;
	    integer table_num = curd->d_table;
	    integer fortran_row = row + 1;
	    integer fortran_col = col + 1;
 ...
 * Requires mapping x & y to row & column.  Mostly used for Fortran	*
 * to-C mapping.							*
 * Used in: greebl							*
 ************************************************************************/
    {
 ...
 ************************************************************************/
    {
    if( open_channels[*chan] == stdout && in_graphics_mode )
...
#ifndef something
    if( default_output_channel==0 && ! in_graphics_mode ) grafon_();
#endif
...
/************************************************************************/
void allcap_( integer *str, integer *len )
/************************************************************************
...
void send_browser( struct display_struct *curd, int serial, char *cp )
    ...
    chtype *oldp = NULL;
    int row, col;
    int size = curd->d_nrows * curd->d_ncols * sizeof(chtype);

    if( curd->d_display_type != DT_array )
	fatal(__FILE__,__LINE__,"Cannot send_browser display.");

    if( curd->d_last == NULL )
        {
	curd->d_last = (chtype*)malloc( size );
	curd->d_succ = (chtype*)malloc( size );
    ...
    memcpy( newp = curd->d_last, curd->d_data, size );
		...
		progress(__FILE__,__LINE__,"Pre get_packet(%d) logic.",chan);
			...
			*cp = 0;
			...
			if( strcmp( url, "/" ) == 0 )
			    ...
			    if( ! webserver_file )
				{
				struct stat stat_html, stat_js;
				int chan_html, chan_js;
				char *namebuf=malloc(strlen(full_progname)+10);
				sprintf( namebuf, "%s.html", full_progname );
				chan_html = open( namebuf, 0 );
				char *cp = strrchr( namebuf, '/' );
				cp = ( cp ? cp+1 : namebuf );
				if( chan_html < 0 )
				    {
				    strcpy( cp, "multis.html" );
				    if( (chan_html = open( namebuf, 0 ) ) < 0 )
					fatal(__FILE__,__LINE__,
					    "Cannot open %s:  %s",
					    namebuf, STRERROR(errno) );
				    }
				if( fstat(chan_html,&stat_html) < 0 )
				    fatal(__FILE__,__LINE__,
				        "Cannot fstat %s:  %s",
					namebuf, STRERROR(errno) );
				strcpy( cp, "multis.js" );
				if( (chan_js = open( namebuf, 0 )) < 0 )
				    fatal(__FILE__,__LINE__,
				        "Cannot open %s:  %s",
					namebuf, STRERROR(errno) );
				if( fstat(chan_js,&stat_js) < 0 )
				    fatal(__FILE__,__LINE__,
				        "Cannot fstat %s:  %s",
					namebuf, STRERROR(errno) );
				webserver_file_length =
				    stat_js.st_size + stat_html.st_size;
				free( namebuf );
				cp = webserver_file =
				    (char*)malloc(webserver_file_length);
				if( read( chan_js, cp, stat_js.st_size )
				    < stat_js.st_size )
				    fatal(__FILE__,__LINE__,
				        "Read js failed:  %s",
					STRERROR(errno) );
				cp += stat_js.st_size;
				if( read( chan_html, cp, stat_html.st_size )
				    < stat_html.st_size )
				    fatal(__FILE__,__LINE__,
				        "Read html failed:  %s",
					STRERROR(errno) );
				...
				webserver_file, webserver_file_length );
			    close( chan );
			...
			progress(__FILE__,__LINE__,"Discarding URL:  [%s]",url);
			    ...
			    if( cp )
			        {
				check_serial = atoi( cp );
				while( ( cp = strtok( NULL, "/" ) ) )
				    {
				    int inchar = atoi(cp);
				    if( inchar > 0 )
					{
					/* Happily overflow ring buffer */
					webserver_command_buf[
					    webserver_command_in_ind++]
					    = inchar;
					webserver_command_in_ind
					    %= MEDIUM_BUFSIZE;
					}
			    ...
			    struct display_struct *curd = get_display(0);
			    char resp[10000];
			    sprintf( resp,
				"HTTP/1.1 200 OK\n"
				...
				"<META FOO=4 HTTP-EQUIV=\"Content-Type\" "
 ...
 ************************************************************************/
    {
    if( webserver_chan )	/* Give chance for to output death message */
        {
	time_t stop_at, now;
	time( &stop_at );
	stop_at+=4;		/* Wait at least 4 seconds */
	do  {
	    webserver_logic();
	    time( &now );
	    } while( now < stop_at );
 ...
 ************************************************************************/
	...
	if( strcmp(*argv,"-dump") == 0 )
	    ...
	    argc--;
	    if( ! *++argv )
		fatal(__FILE__,__LINE__,"No uid specified with -webserver");
	    run_as_uid = *argv;
	    argc--;
	    }
	else if( argv[0][0] == '-' )
	...
	if( run_as_uid )
	    progress(__FILE__,__LINE__,
	        "Wrong UID (%d vs %d) trying to run as %s.",
		    getuid(), PRIVILEGED_UID, run_as_uid );
	else
	    progress(__FILE__,__LINE__,"Turning off setuid (%d vs %d).",
	        getuid(), PRIVILEGED_UID );
	seteuid( getuid() );
	}
    else
	{
	progress(__FILE__,__LINE__,"Potentially changing uid...");
	struct group *gamers = getgrnam("multis");
	if( gamers )	/* If multis exists, make sure player is in group */
	    {
	    char **gp = gamers->gr_mem;
	    while( *gp && strcmp(*gp,run_as_uid) ) gp++;
	    if( ! * gp )
		fatal(__FILE__,__LINE__,
		    "%s is not a member of multis", run_as_uid);
	...
	progress(__FILE__,__LINE__,"Reading password file for %s...",
	    run_as_uid );
	struct passwd *mypw = getpwnam( run_as_uid );
	if( ! mypw )
	    fatal(__FILE__,__LINE__,"Cannot get passwd entry for %s",*argv);
	if( setuid( mypw->pw_uid ) < 0 )
	    fatal(__FILE__,__LINE__,"setuid(%d) failed:  %s",
		mypw->pw_uid, STRERROR(errno) );
	if( setuid( mypw->pw_uid ) < 0 )
	    fatal(__FILE__,__LINE__,"seteuid(%d) failed:  %s",
		mypw->pw_uid, STRERROR(errno) );
	...
	open_listener( webserver_port,
	    &webserver_portnum, &webserver_chan, &webserver_fds );
